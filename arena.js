/* Territory v145 — Arena rebuilt from the video reference + agreed Territory rules.
...
(()=>{
  ...
  const VIP_KEY='territory_vip_v5';
  const QUEST_KEY='territory_alex_vip_quest_v5';
  const VIP_DAYS=10, DAY=86400000, QUEST_TARGET=5;
  const readJson=(k,d)=>{try{const v=JSON.parse(localStorage.getItem(k));return v??d}catch(e){return d}};
  const hasVIP=()=>{
    const s=getState(), v=readJson(VIP_KEY,null);
    const until=Math.max(Number(s.vipUntil||0),Number(v?.until||0));
    return Boolean(s.vip) || until>Date.now();
  };
  const vipDays=()=>{
    const s=getState(), v=readJson(VIP_KEY,null);
    const until=Math.max(Number(s.vipUntil||0),Number(v?.until||0));
  ...
  const questState=()=>{const q=readJson(QUEST_KEY,null);return q&&typeof q==='object'?q:{accepted:false,startWins:stats.wins,completed:false,rewardClaimed:false}};
  const saveQuest=q=>localStorage.setItem(QUEST_KEY,JSON.stringify(q));
  const questProgress=()=>Math.min(QUEST_TARGET,Math.max(0,stats.wins-Number(questState().startWins||0)));
  function openAlexQuest(){
    let q=questState();
    if(!q.accepted){q={accepted:true,startWins:stats.wins,completed:false,rewardClaimed:false};saveQuest(q)}
    const progress=questProgress(), done=progress>=QUEST_TARGET || q.completed;
    showModal('👑 Алекс — задание',`<div class="arena140 arena140-result">
      <div class="result-icon">👑</div><h2>Задание Алекса</h2>
      <p>Докажи свою силу на Арене: одержи 5 новых побед.</p>
      <div class="arena140-reward">Прогресс: ${progress}/${QUEST_TARGET}</div>
      ${done&&!q.rewardClaimed?'<button class="arena140-primary" data-vip-claim>Получить VIP на 10 дней</button>':''}
      ${q.rewardClaimed?'<div class="arena140-reward">VIP активен · '+vipDays()+' дн.</div>':''}
      <button class="arena140-secondary" data-vip-back>Вернуться в Arena</button>
    </div>`);
    body().querySelector('[data-vip-claim]')?.addEventListener('click',()=>{
      const until=Date.now()+VIP_DAYS*DAY;
      localStorage.setItem(VIP_KEY,JSON.stringify({until}));
      const st=getState();st.vip=true;st.vipUntil=until;save();
      q.rewardClaimed=true;q.completed=true;saveQuest(q);
      window.arenaToast('👑 VIP получен на 10 дней');openAlexQuest();
    });
    body().querySelector('[data-vip-back]')?.addEventListener('click',renderHome);
  }
  function hookAlex(){
    const run=()=>document.querySelectorAll('.guard-label[aria-label*="Alex"]').forEach(el=>{
      if(el.dataset.arenaAlexHooked)return;
      el.dataset.arenaAlexHooked='1';
      el.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openAlexQuest()},true);
    });
  ...
  function renderHome(){
    ...
    showModal('⚔️ Арена',`<div class="arena140">
      ...
      <section class="arena140-rules"><b>👑 VIP / Задание Алекса</b><p>${hasVIP()?'VIP активен · осталось '+vipDays()+' дн.':'Алекс: '+questProgress()+'/5 новых побед · открыть задание'}</p><div><button class="arena140-secondary" data-alex-quest>${hasVIP()?'Посмотреть VIP':'Открыть задание Алекса'}</button></div></section>
    ...
    body().querySelector('[data-alex-quest]')?.addEventListener('click',openAlexQuest);
    hookAlex();
  }
  ...
  function renderBattle(){
    ...
    showModal('⚔️ Arena · бой',`<div class="arena140 arena140-combat">
      ...
      <section class="arena140-finish"><button data-finish>Завершить бой</button><button data-extend>Продлить +5 мин</button><button data-autobattle ${hasVIP()?'':'disabled'}>${hasVIP()?'🤖 Автобой':'🔒 Автобой · VIP'}</button></section>
    </div>`);
    ...
    body().querySelector('[data-autobattle]')?.addEventListener('click',()=>{
      if(!hasVIP())return window.arenaToast('Автобой доступен после получения VIP');
      if(battle.auto)return;
      battle.auto=true; window.arenaToast('🤖 Автобой включён');
      const autoTick=()=>{if(!battle||battle.ended||!battle.auto)return; battle.attack=ATTACK_ZONES[Math.floor(Math.random()*ATTACK_ZONES.length)][0]; battle.defense=[...DEF_ZONES].sort(()=>Math.random()-.5).slice(0,2).map(z=>z[0]); resolveTurn(); if(battle&&!battle.ended)setTimeout(autoTick,900)};
  ...
  function finishBattle(result){
    ...
    try{window.territorySystems?.onBattleFinished?.(win?'win':result==='Поражение'?'loss':'finish')}catch(e){}
