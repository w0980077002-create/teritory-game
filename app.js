...
$("#shopGrid").addEventListener("click",e=>{
 ...
 if((window.TerritoryEconomy? !window.TerritoryEconomy.canAfford('coins',cost):state.coins<cost)){const l=$("#merchantLog");if(l)l.textContent="Торговец: «Не хватает монет.»";return;}
 if(window.TerritoryEconomy)window.TerritoryEconomy.spend('coins',cost,'market-purchase'); else state.coins-=cost;state.weapon=w.name;state.bonusDamage=w.damage;state.inventory.push(w.icon);state.merchantRep+=1;
