...
function ensurePlayer(id,name){if(!db.players[id])db.players[id]={id,name:name||'Игрок',username:'',photoUrl:'',firstName:'',lastName:'',clan:'',level:1,xp:0,updatedAt:Date.now()};else{const p=db.players[id];if(p.username==null)p.username='';if(p.photoUrl==null)p.photoUrl='';if(p.firstName==null)p.firstName='';if(p.lastName==null)p.lastName='';}return db.players[id]}
function syncTelegramProfile(id,a,name){if(!id)return null;const p=ensurePlayer(id,name||a?.user?.first_name||'Игрок'),u=a?.user||{};p.name=clean(name||[u.first_name,u.last_name].filter(Boolean).join(' ')||u.username||p.name||'Игрок');p.username=clean(u.username||p.username||'',64);p.photoUrl=clean(u.photo_url||p.photoUrl||'',500);p.firstName=clean(u.first_name||p.firstName||'',64);p.lastName=clean(u.last_name||p.lastName||'',64);p.lastSeenAt=Date.now();p.updatedAt=Date.now();return p}
function isPlayerOnline(id){const now=Date.now(),sid=String(id);return [...players.values()].some(x=>String(x.telegramId||x.playerId)===sid)||Object.values(db.players).some(p=>String(p.id)===sid&&Number(p.lastSeenAt||0)>now-30000)}
function clanByMember(id){return Object.values(db.clans).find(c=>c.members&&c.members[id])||null}
function snapshot(room){const set=rooms.get(room);return {room,players:set?[...set].map(ws=>{for(const p of players.values())if(p.ws===ws){const d=db.players[p.telegramId||p.playerId]||{};return {playerId:p.telegramId||p.playerId,name:p.name,username:d.username||'',photoUrl:d.photoUrl||'',level:Number(d.level||1),connected:true}}return null}).filter(Boolean):[]}}
...
const server=http.createServer((req,res)=>{
  ...
  if(u.pathname==='/api/profile'&&req.method==='POST')return body(req,b=>{const {a,id}=playerIdFrom(req,b);if(!a.ok&&BOT_TOKEN)return json(res,401,{ok:false,error:'telegram_auth_failed'});if(!id)return json(res,400,{ok:false,error:'telegram_id_required'});const old=db.players[id]||{};const p=syncTelegramProfile(id,a,clean(b.name||old.name||a.user?.first_name||'Игрок'));p.clan=clean(b.clan||old.clan||'',32);p.level=Number(b.level||old.level||1);p.xp=Number(b.xp??old.xp??0);persist();return json(res,200,{ok:true,profile:p})});
  if(u.pathname==='/api/presence'&&req.method==='POST')return body(req,b=>{const {a,id}=playerIdFrom(req,b);if(!a.ok&&BOT_TOKEN)return json(res,401,{ok:false,error:'telegram_auth_failed'});if(!id)return json(res,400,{ok:false,error:'telegram_id_required'});const p=syncTelegramProfile(id,a,b.name||'Игрок');persist();return json(res,200,{ok:true,player:{id:p.id,name:p.name,username:p.username,photoUrl:p.photoUrl,level:Number(p.level||1),online:true,lastSeenAt:p.lastSeenAt}})});
  ...
  const mmPublic = q => {const p=db.players[q.playerId]||{};return {playerId:q.playerId,name:q.name||p.name,rating:q.rating,joinedAt:q.joinedAt,status:q.status||'searching',matchId:q.matchId||null,opponent:q.opponent||null,username:p.username||'',photoUrl:p.photoUrl||'',level:Number(p.level||1),online:true};};
  ...
  if(u.pathname==='/api/matchmaking/join'&&req.method==='POST')return body(req,b=>{
    ...
    const p=syncTelegramProfile(id,a,clean(b.name||a.user?.first_name||'Игрок'));p.name=clean(b.name||p.name||'Игрок');
  ...
  if(u.pathname==='/api/player'&&req.method==='GET'){
    ...
    return json(res,200,{ok:true,player:{id:p.id,name:p.name,username:p.username||'',photoUrl:p.photoUrl||'',firstName:p.firstName||'',lastName:p.lastName||'',clan:p.clan||'',level:p.level||1,xp:p.xp||0,online:isPlayerOnline(id)}});
  }
  ...
  if(u.pathname==='/api/clan/invite'&&req.method==='POST')return body(req,b=>{
    ...
    // S84 FRIENDS & SOCIAL
  function socialPlayer(id){const p=db.players[id]||{};return {id:p.id,name:p.name||'Игрок',username:p.username||'',photoUrl:p.photoUrl||'',clan:p.clan||'',level:Number(p.level||1),xp:Number(p.xp||0),online:isPlayerOnline(id)};}
