...
export class GameHub extends DurableObject {
  ...
  async action(pid, name, a) {
    ...
    if (type === "battle.start") {
      ...
      if (state.battle && !state.battle.finished) return json({ ok: false, error: "battle_in_progress" }, 409);

      const enemyIndex = Math.min(ENEMIES.length - 1, Math.floor((state.wins || 0) / 3));
      ...
      return json({
        ok: true,
        player: state,
        battle: { id: state.battle.id, turn: 1, enemy, enemyHp: enemy.hp, playerHp: state.hp }
      });
    }

    if (type === "battle.turn") {
      if (!validMove(a)) return json({ ok: false, error: "invalid_move" }, 400);
      if (!state.battle || state.battle.finished) return json({ ok: false, error: "battle_not_active" }, 409);

      const battle = state.battle;
      const enemy = ENEMIES[Math.max(0, Math.min(ENEMIES.length - 1, Number(battle.enemyIndex) || 0))];
      ...
      battle.enemyHp = Math.max(0, Number(battle.enemyHp) - playerDamage);
      battle.playerHp = Math.max(0, Number(battle.playerHp) - enemyDamage);
      state.hp = battle.playerHp;
      const result = battle.enemyHp <= 0 ? "win" : battle.playerHp <= 0 ? "loss" : "continue";
      battle.finished = result !== "continue";
      battle.turn += 1;
      ...
      return json({
        ...
        player: state,
        round: {
          turn: battle.turn - 1, enemyHp: battle.enemyHp, playerHp: battle.playerHp,
          enemyAttack, enemyDefs, attack, defs, hit: playerHitsEnemy, dodge, blocked,
          playerDamage, enemyDamage, result
        },
        battle: { enemy, enemyHp: battle.enemyHp, playerHp: battle.playerHp, turn: battle.turn, finished: battle.finished }
      });
    }

    if (type === "battle.reward") {
      if (!state.battle || !state.battle.finished || state.battle.enemyHp > 0) {
        return json({ ok: false, error: "battle_reward_not_ready" }, 400);
      }
      const enemy = ENEMIES[Math.max(0, Math.min(ENEMIES.length - 1, Number(state.battle.enemyIndex) || 0))];
    ...
    if (type === "battle.loss") {
      if (!state.battle || !state.battle.finished || state.battle.playerHp > 0) {
        return json({ ok: false, error: "battle_loss_not_ready" }, 400);
      }
      state.losses++;
      state.battles++;
