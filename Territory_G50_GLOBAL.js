/* Territory G50 GLOBAL — PvE battle reliability pass over the verified G48 base + G49 Arena patch. */
(function(){'use strict';
 ...
 function renderPve(host){
   ...
   host.innerHTML=`<div class="tg-route-compact"><b>${pve.boss?'👑 БОСС ГОРОДА':'⚔️ ГОРОДСКОЙ PvE-БОЙ'}</b><span>${pve.boss?'Переход на следующий город':'Прогресс '+st.pveProgress+'%'}</span></div><div class="tg-battle-stage ${pve.boss?'tg-boss-stage':''}"><div class="tg-battle-top"><span>${pve.boss?'👑 БОСС':'🤖 БОТ'}</span><b>Раунд ${pve.round||1}</b>${timer}</div><div class="tg-fighters"><div class="tg-fighter hero"><div class="tg-fighter-ico">🧔</div><b>${esc(st.name)}</b><span>HP ${st.hp}/${st.maxHp}</span><div class="tg-bar tg-hp"><i style="width:${Math.max(0,st.hp/st.maxHp*100)}%"></i></div></div><div class="tg-vs">VS</div><div class="tg-fighter enemy"><div class="tg-fighter-ico">${e.ico}</div><b>${e.name}</b><span>HP ${Math.max(0,e.hp)}/${e.maxHp}</span><div class="tg-bar tg-hp"><i style="width:${pct}%"></i></div><small>Урон ${e.damage}</small></div></div></div><div class="tg-card tg-pve-actions"><div class="tg-card-head"><b>${pve.boss?'Победи босса до окончания таймера':'Твой ход'}</b><span>⚔️ ${st.combatStone}</span></div><p class="tg-muted">Это городской PvE: ты сражаешься с ботом или боссом по одному ходу. Команд, комнат и PvP-механик Arena здесь нет.</p><button class="tg-btn gold tg-pve-main-hit" data-pve-hit ${st.combatStone<1?'disabled':''}>⚔️ УДАР · 1 Боевой камень</button></div>${pveLogHtml()}<div class="tg-card tg-reward-preview"><b>${pve.boss?'🏆 Награда босса':'🎁 Награда за бота'}</b><span>🪙 ${e.reward} · ⭐ ${e.xp} XP</span></div>`;
 ...
 function pveHit(host){
   if(!pve.enemy||st.combatStone<1)return;
   ...
   const armorBonus=eq.defense;let enemyHit=Math.max(0,Math.round(pve.enemy.damage-(st.defense*1.5)-armorBonus));
