...
(function(){'use strict';
 ...
 function level(){return Number(st.cityLevel||1)}
 ...
 function render(){
  ...
  grid.innerHTML=districts.map(d=>{const locked=lv<d.req;return `<button type="button" class="g66-point ${locked?'locked':''}" data-g66="select" data-id="${d.id}"><i>${d.icon}</i><b>${esc(d.name)}</b><small>${locked?'🔒 Ур. '+d.req:'ДОСТУПЕН'}</small></button>`}).join('');
  select('center');
 }
 function select(id){
 ...
 .g66-close{float:right;width:42px;height:42px;border:0;border-radius:50%;background:rgba(255,255,255,.1);color:#fff;font-size:20px}.g66-head{padding:4px 4px 14px}.g66-head small{opacity:.65}.g66-head h2{margin:3px 0 2px;font-size:25px}.g66-head span{opacity:.75}.g66-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.g66-point{min-height:94px;border:1px solid rgba(255,255,255,.12);border-radius:15px;background:rgba(255,255,255,.06);color:#fff;text-align:left;padding:11px;display:grid;grid-template-columns:38px 1fr;grid-template-rows:1fr auto;column-gap:8px}.g66-point i{font-style:normal;font-size:27px;grid-row:1/3;align-self:center;text-align:center}.g66-point b{font-size:14px;align-self:end}.g66-point small{opacity:.6;font-size:11px;margin-top:5px}.g66-point.selected{border-color:rgba(255,210,100,.7);background:rgba(255,190,60,.1)}.g66-point.locked{opacity:.48}.g66-info{margin-top:11px;padding:14px;border-radius:16px;background:rgba(0,0,0,.2)}.g66-info small{opacity:.6}.g66-info h3{margin:4px 0 5px}.g66-info p{margin:0 0 11px;opacity:.78;line-height:1.4}.g66-go{width:100%;height:46px;border:0;border-radius:12px;background:#d6a33a;color:#111;font-weight:800}.g66-go:disabled{opacity:.45}.g66-log{margin-top:9px;min-height:22px;opacity:.7;font-size:12px}
