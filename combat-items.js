...
function render(){
  ...
  normalize();
  grid.querySelectorAll('.ci-item,.ci-empty,.ci-title').forEach(x=>x.remove());
  const ids=['elixir_hp','elixir_energy','elixir_guard','elixir_attack'];
  const cards=ids.map(id=>{
    const c=Object.values(CATALOG).find(v=>v.id===id), q=quantity(id);
    return `<button type="button" class="ci-item${q<=0?' ci-zero':''}" data-ci="${esc(id)}" ${q<=0?'aria-disabled="true"':''}><span class="ci-item-icon">${c.icon}</span><b>${esc(c.name)}</b><small>×${q}</small></button>`;
  }).join('');
  grid.insertAdjacentHTML('beforeend',`<div class="ci-title">БОЕВЫЕ ПРЕДМЕТЫ</div>${cards}`);
