...
(function(){'use strict';
 ...
 function renderPve(host){
   if(!host)return;
   if(!pve.enemy&&!pve.started)pveBegin(host);
   if(!pve.enemy){
     host.innerHTML=`<div class="tg-card tg-result"><b>🏙️ Уровень города ${st.cityLevel}</b><p>Путь завершён. Подготовься к следующей попытке.</p><button class="tg-btn gold" data-pve-retry>Продолжить путь</button></div>`;return;
   }
   const e=pve.enemy,eq=equipmentTotals(),pct=Math.max(0,Math.min(100,Math.round(e.hp/e.maxHp*100))),next=Math.min(100,st.pveProgress+20);
   const remain=pve.boss?Math.max(0,pve.bossEndsAt-Date.now()):0;
   const timer=pve.boss?`<span class="tg-boss-timer">⏱️ ${Math.floor(remain/60000)}:${String(Math.floor(remain/1000)%60).padStart(2,'0')}</span>`:'';
   const stage=pve.boss?'БОСС':`БОТ ${Math.floor(st.pveProgress/20)+1}/5`;
   host.innerHTML=`
    <div class="tg-card tg-city-progress"><div class="tg-card-head"><b>🏰 Sdolars · город ${st.cityLevel}</b><span>${stage}</span></div>${pveRoute()}<div class="tg-bar tg-progress"><i style="width:${st.pveProgress}%"></i></div><div class="tg-progress-label"><span>${st.pveProgress}%</span><span>⚔️ Боевые камни: ${st.combatStone}</span></div></div>
    <div class="tg-battle-stage ${pve.boss?'tg-boss-stage':''}">
      <div class="tg-battle-top"><span>${pve.boss?'👑 БОСС ГОРОДА':'🤖 Последовательный противник'}</span><b>Раунд ${pve.round||1}</b>${timer}</div>
      <div class="tg-fighters">
       <div class="tg-fighter hero"><div class="tg-fighter-ico">🧔</div><b>${esc(st.name)}</b><span>HP ${st.hp}/${st.maxHp}</span><div class="tg-bar tg-hp"><i style="width:${Math.max(0,st.hp/st.maxHp*100)}%"></i></div><small>⚔️ +${eq.damage} · 🛡️ +${eq.defense}</small></div>
       <div class="tg-vs">VS</div>
       <div class="tg-fighter enemy"><div class="tg-fighter-ico">${e.ico}</div><b>${e.name}</b><span>HP ${Math.max(0,e.hp)}/${e.maxHp}</span><div class="tg-bar tg-hp"><i style="width:${pct}%"></i></div><small>Урон ${e.damage}</small></div>
      </div>
    </div>
    <div class="tg-card"><div class="tg-card-head"><b>${pve.boss?'👑 Победи босса за переход':'🤖 Победи противника и двигайся вправо'}</b><span>${pve.boss?'Таймер активен':`${st.pveProgress}% → ${next}%`}</span></div>
      <p class="tg-muted">Игрок сам выбирает действие. Бой не идёт автоматически.</p>
      <b>1. Атака — выбери 1 зону</b><div class="tg-zones" style="margin-top:8px">${Object.entries(zoneNames).map(([id,n])=>`<button class="tg-zone ${pve.attack===id?'sel-a':''}" data-pve-a="${id}">${n}</button>`).join('')}</div>
      <b style="display:block;margin-top:12px">2. Защита — выбери ровно 2 зоны</b><div class="tg-zones" style="margin-top:8px">${Object.entries(zoneNames).map(([id,n])=>`<button class="tg-zone ${pve.def.includes(id)?'sel-d':''}" data-pve-d="${id}">${n}</button>`).join('')}</div>
      <div class="tg-battle-actions"><button class="tg-btn gold" data-pve-hit ${!pve.attack||pve.def.length!==2||st.combatStone<1?'disabled':''}>⚔️ Удар · 1 Боевой камень</button><span>⚔️ ${st.combatStone}</span></div>
    </div>
    ${pveLogHtml()}
    <div class="tg-card tg-reward-preview"><b>${pve.boss?'🏆 Победа над боссом':'🎁 Победа над ботом'}</b><span>🪙 ${e.reward} · ⭐ ${e.xp} XP ${pve.boss?'· следующий город':'· прогресс +20%'}</span></div>`;
 }
 ...
 function pveHit(host){
   ...
   const enemyDef=pickDefense();let dmg=Math.max(4,5+st.strength+st.weaponMastery+eq.damage);if(pve.attack==='head')dmg+=3;if(pve.attack==='legs')dmg=Math.max(3,dmg-1);
   const crit=Math.random()<Math.min(.6,st.strength*.02);if(crit)dmg=Math.round(dmg*1.6);
   const blocked=enemyDef.includes(pve.attack);if(blocked)dmg=Math.max(1,Math.round(dmg*.35));
   pve.enemy.hp=Math.max(0,pve.enemy.hp-dmg);
   pve.log.push(`Раунд ${pve.round}: ты атаковал «${zoneNames[pve.attack]}»${crit?' — КРИТ':''}. Бот защищал ${enemyDef.map(z=>zoneNames[z]).join(' и ')}${blocked?' — удар частично заблокирован':''}. Урон: ${dmg}.`);
   if(pve.enemy.hp<=0){
